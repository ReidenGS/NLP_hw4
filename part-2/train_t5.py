...
def get_args():
    ...
    parser.add_argument('--num_beams', type=int, default=1,
                        help="Beam size for generation. Use 1 for faster training-time eval and a larger value for final decoding.")
    parser.add_argument('--max_new_tokens', type=int, default=256,
                        help="Maximum number of new tokens to generate for SQL decoding.")
    parser.add_argument('--eval_every', type=int, default=1,
                        help="Run generation-based dev evaluation every N epochs. Intermediate epochs only compute loss.")
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path, force_generation=False):
    '''
    ...
    '''
    global _eval_call_count, _cached_metrics
    _eval_call_count += 1
    ...
    run_generation = force_generation or (_eval_call_count % max(args.eval_every, 1) == 0)
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, initial_decoder_inputs in tqdm(dev_loader):
            ...
            non_pad_tokens = (decoder_targets != PAD_IDX).sum().item()
            ...
            if run_generation:
                ...
                decoded_sql = tokenizer.batch_decode(generated_ids, skip_special_tokens=True)
                generated_sql_queries.extend([x.strip() for x in decoded_sql])

    avg_loss = total_loss / max(total_tokens, 1)
    if not run_generation:
        model.train()
        _, last_f1, last_em, last_sql_em, last_error_rate = _cached_metrics
        return avg_loss, last_f1, last_em, last_sql_em, last_error_rate
    ...
    _cached_metrics = (avg_loss, record_f1, record_em, sql_em, error_rate)
    model.train()
