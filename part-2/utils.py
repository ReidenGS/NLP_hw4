...
def compute_records(processed_qs: List[str]):
    ...
    num_threads = min(32, len(processed_qs))
    timeout_secs = 30

    rec_dict = {}
    with ThreadPoolExecutor(max_workers=num_threads) as pool:
        future_to_query_id = {
            pool.submit(compute_record, i, query, timeout_secs): i
...
def compute_record(query_id, query, timeout_secs=30):
    ...
    try:
        ...
        rec = []
        if "interrupted" in str(e).lower() and time.monotonic() >= deadline:
