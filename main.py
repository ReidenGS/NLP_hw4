...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    #begin the training mode
    model.train()
    #loop the train dataloader
...
def create_augmented_dataloader(args, dataset):
    ...
    augmented_train_dataset = datasets.concatenate_datasets([train_dataset, transformed_subset]).shuffle(seed=42)
    ...
    if args.debug_train:
        augmented_train_dataset = augmented_train_dataset.select(range(min(4000, len(augmented_train_dataset))))
